...
export const onRequest: MiddlewareHandler = async (context, next) => {
  ...
  // Get the response from next middleware/page
  const response = await next();

  // 🚀 AMP POST-PROCESSING - Transform HTML for AMP pages
  ...
  if (isAmpRoute) {
    ...
    if (contentType && contentType.includes('text/html')) {
      try {
        ...
        // Check if we need to process this HTML
...
function transformAmpHTML(html: string): string {
  ...
  while ((existingAmpMatch = existingAmpCustomRegex.exec(html)) !== null) {
    ...
    processedHTML = processedHTML.replace(existingTag, `<!-- AMP: Merged existing amp-custom -->`);
  ...
  while ((match = styleRegex.exec(html)) !== null) {
    ...
    if (cssContent) {
      console.log(`🎨 Found CSS module style: ${cssContent.substring(0, 80)}...`);
      stylesToRemove.push({
        tag: fullTag,
        content: cssContent
      });
  ...
  stylesToRemove.forEach((style, index) => {
    processedHTML = processedHTML.replace(style.tag, `<!-- AMP: Inlined style block ${index + 1} -->`);
  ...
  linksToRemove.forEach((link, index) => {
    ...
    processedHTML = processedHTML.replace(link, `<!-- AMP: Removed CSS link ${index + 1} -->`);
    processedCount++;
  });
